...
async function run(req) {
  ...
  req.stream.write('generating design task ...');
  ...
  const stream = await replitai.chat({
    ...
    messages: [
      {
        author: 'system',
        content: `Your task is to modify a ${_titleCase(
          req.query.framework,
        )} component for a web app, according to the user's request.\n` +
          `If you judge it is relevant to do so, you can specify pre-made library components to use in the component update.\n` +
          `You can also specify the use of icons if you see that the user's update request requires it.` +
          ...
          `Design the ${_titleCase(
            req.query.framework,
          )} web component updates for the user, as the creative genius you are. Your answer should start and end with curly brackets.`,
      },
    ],
  });
  ...
  */
  ...
  const component_design = {
    ...
    ...JSON.parse(stream.value.message.content),
  };
  ...
  req.stream.write(`${JSON.stringify(component_task)}\n`);
