  ...
  if (req.pipeline.stages[`component-validation-check`].success) {
    ...
    const errors_context_entries = req.pipeline.stages[
      ...
      return {
        role: `user`,
        content:
          `# Component Error (${_error_idx + 1}/${req.pipeline.stages[`component-validation-check`].data
    ...
    const context = [
      {
        ...
        content:
          ...
          `The fixed code you generate will be directly written to a .${FRAMEWORKS_EXTENSION_MAP[req.query.framework]
          } ${_titleCase(
            req.query.framework,
