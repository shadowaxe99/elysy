...
function App() {
  ...
  async function shareComponent() {
    ...
    const response = await fetch(
      `https://${__REPL_SLUG__}.${__REPL_OWNER__}.repl.co:8099/components/share`,
      {
        method: "POST",
        headers: {
  ...
  async function fetchComponents() {
    ...
    setLoadedComponents([...[]])
    const response = await fetch(
      `https://${__REPL_SLUG__}.${__REPL_OWNER__}.repl.co:8099/components/get?framework=react&components=shadcn&icons=lucide&name=${name}`
    );
  ...
  const iterateComponent = async () => {
    ...
    let received_stream = ''
    const response = await fetch(
      `https://${__REPL_SLUG__}.${__REPL_OWNER__}.repl.co:8099/components/iterate/description`,
      {
        method: "POST",
        headers: {
          'Accept': 'application/json',
