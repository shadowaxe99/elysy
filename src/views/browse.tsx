...
export default function Browse() {
  ...
  async function fetchComponents() {
    const response = await fetch(
      `https://${__REPL_SLUG__}.${__REPL_OWNER__}.repl.co:8099/components/list?framework=react&components=shadcn&icons=lucide`
    );
  ...
  const newComponent = async () => {
    ...
    const response = await fetch(
      `https://${__REPL_SLUG__}.${__REPL_OWNER__}.repl.co:8099/components/new/${generateMode}`,
      {
        method: "POST",
        headers: {
